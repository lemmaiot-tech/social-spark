...
const EditBrandContext: React.FC<EditBrandContextProps> = ({ initialContext, onSave, onBack }) => {
  ...
  useEffect(() => {
    if (initialContext) {
      setBrandName(initialContext.brandName || '');
      setBio(initialContext.bio || '');
      const examples = initialContext.postExamples || [];
      // Ensure there are at least 2 empty strings for the input fields
      const paddedExamples = [...examples, '', ''].slice(0, 2);
      setPostExamples(paddedExamples);
      setBrandVoice(initialContext.brandVoice || '');
    }
  }, [initialContext]);
    ...
    <div className="animate-fade-in-up">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2 flex items-center justify-center gap-2">
            <UserIcon /> Edit Your Brand Profile
        </h2>
        <p className="text-center text-gray-600 dark:text-gray-400">
            Keep this updated to ensure the AI's suggestions stay relevant to your brand.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-between items-center pt-2">
          <button
            type="button"
            onClick={onBack}
            className="px-4 py-2 text-sm font-medium text-gray-700 bg-transparent rounded-md hover:bg-gray-100 focus:outline-none dark:text-gray-300 dark:hover:bg-gray-700"
          >
            Back
          </button>
          <button
...
export default EditBrandContext;